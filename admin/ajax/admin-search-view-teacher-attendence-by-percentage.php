...
$unixEndDate = strtotime($_POST['enddate']);    
    ...
    else
    {
        ...
        if(!empty($_POST['scnic'] ) && !empty($_POST['startdate']) && !empty($_POST['enddate']) )
        {
            ...
            $query = "select * from teacher where cnic = '$scnic'";
...
?>
    ...
    $query2 = "select * from teacher_attendence where t_id = " . $row['t_id'] . " and date >= '$unixStartDate' and date <= '$unixEndDate' ";
    ...
    if($result2)
    {
        ...
        $query4 = "select * from teacher_attendence where t_id = " . $row['t_id'] . " and status = 'leave' and date >= '$unixStartDate' and date <= '$unixEndDate' ";
        $result4 = mysqli_query($con, $query4);

        $query5 = "select * from teacher_attendence where t_id = " . $row['t_id'] . " and status = 'present' and date >= '$unixStartDate' and date <= '$unixEndDate' ";
        $result5 = mysqli_query($con, $query5);

        $query6 = "select * from teacher_attendence where t_id = " . $row['t_id'] . " and status = 'absent' and date >= '$unixStartDate' and date <= '$unixEndDate' ";
